...
static bool h264_update(struct h264_encoder *enc, obs_data_t *settings)
{
	...
	const char *rc_mode = "bitrate"; // We only want to use bitrate mode
	AVDictionary *oh264_opts = NULL;
	...
	av_dict_set(&oh264_opts, "rc_mode", rc_mode, 0);
	av_dict_set(&oh264_opts, "profile", profile, 0);

	const char *ffmpeg_opts = obs_data_get_string(settings, "ffmpeg_opts");
	ffmpeg_video_encoder_update(&enc->ffve, bitrate, keyint_sec, voi, &info,
				    ffmpeg_opts);
	av_dict_free(&oh264_opts);
